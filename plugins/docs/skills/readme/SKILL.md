...
- **Tree output is too large in monorepos** -- `generate_tree.sh` can produce 500+ lines in monorepos with many packages, especially when some packages have their own `node_modules` that are not gitignored. The resulting Project Structure section becomes unreadable. Trim the tree aggressively: limit depth to 2 for the top level, exclude `node_modules` and other dependency directories, and consider showing only the root layout plus one representative package. If the raw output exceeds ~60 lines, manually curate it before pasting into the README.
- **Multiple conflicting languages in monorepo** -- The language detection in Step 1 may find several package manifests (e.g., `package.json` with TypeScript at the root, `pyproject.toml` in one package, `go.mod` in another). Defaulting to whichever manifest is found first gives a misleading picture of the project. When multiple languages are detected, list all of them in the Overview and Features sections rather than picking one. Frame the project as a polyglot monorepo and describe each package's language and role individually.
