...
> 4. **Fact-check the core claims** (only if learning_value is high): pick the 2–4 *load-bearing* claims, search the web to confirm or refute each, and return for each: the claim, a verdict (`confirmed` / `disputed` / `unverifiable`), and a source URL + one-line basis. Flag any claim that is false or seriously disputed.
